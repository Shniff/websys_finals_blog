...
<body>
  ...
  <header>
    ...
    <a href="<?php echo isset($_SESSION['user_id']) ? 'user.php' : 'index.php'; ?>" class="logo-link">
        <h4>CodeX</h4>
    </a>
</div>
      ...
      <div class="menu">
        <?php if (isset($_SESSION['user_id'])): ?>
          ...
          <!-- Notifications Link -->
          <a href="notification.php" <?php echo $current_page == 'notification.php' ? 'class=active' : ''; ?>hidden>Notifications</a>
  ...
  <script>
    $(document).ready(function() {
      ...
      source: function(request, response) {
        $.ajax({
        url: `${window.location.origin}/getters/GetTitle.php`,
        method: 'GET',
        dataType: 'json',
        ...
        success: function(data) {
          ...
          return {
            label: item.title, // Ensure the title is displayed in the dropdown
            value: item.title,
            id: item.post_id
          };
          }));
      ...
      select: function(event, ui) {
        window.location.href = '/app/posts/view_post.php?post_id=' + ui.item.id;
      }
      });
    });
  </script>
